...
%% Electricity analysis data set
% https://archive.ics.uci.edu/ml/datasets/ElectricityLoadDiagrams20112014

if ~exist('LD20112014','var')
    IMPORT_electricity;
end
X = table2array(LD20112014(:,2:end));

N = 1000;

X(:,any(isnan(X))) = [];
x = mean(X(500+(1:N),1:3:end),2);
y = mean(X(500+(1:N),2:3:end),2);
X = [x,y];
t = 1*(1:size(x,1))';
t = mod(t,24*4);
...
a = sgolayfilt(x,sgorder,sgframelen);
b = sgolayfilt(y,sgorder,sgframelen);
