...
y(1) = rand;
for n = 1:N-1
    x(n+1) = 3.82*x(n)*(1-x(n));
    y(n+1) = 3.74*y(n)*(1-y(n));
end
a = 0.015*x + 0.0003*t; % Trend
b = y;
...
for s = 1:NoSurro
    ...
    bsurro(:,s) = awgn(surrogate(b),60,'measured'); %fixing the numerical issue
end
...
fprintf('System\tCCM\tAF_x\tRF_x\tAF_y\tRF_y\n%s\t%s\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t\n',sysname,ccmstring,AFa,RFa,AFb,RFb)
